...
class AppointmentController {
  ...
  async store(req, res) {
    ...
    if (checkAvailability) {
      return res.status(400).json({ error: 'Apointment date is not availble' });
    }

    const appoitnment = await Appointment.create({
      user_id: req.userId,
      provider_id,
      // grants the start of the hour
      date: hourStart,
    });
  ...
  async delete(req, res) {
    ...
    /**
     * Check if date is more or equal to 2 hours to the appointment
     */
